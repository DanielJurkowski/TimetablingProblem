def more_than_one_lesson(solution, weight):
    cost = 0
    ...
    for period in range(solution.number_periods):
        for day in range(solution.number_days):
            ...
            cost += (len(teachers_in_one_period) - len(set_teachers_in_one_period)) + (len(rooms_in_one_period) - len(set_rooms_in_one_period))
...
def free_periods_in_day(solution, weight):
    cost = 0
    ...
    # sprawdzenie dla nauczyciela z jego macierzy wolnych terminów

    return weight * cost


def min_and_max_lessons_in_day(solution, weight):
    cost = 0
